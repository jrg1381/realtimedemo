export default class RealtimeAPIServiceV1 {
  /* @ngInject */
  constructor (
    $rootScope,
    $window,
  ...
  startRecognition (languageCode, sampleRate, proteusConfiguration) {
  ...
  sendConfiguration (transcription_config) {
    if (this.socket.readyState !== this.WebSocket.OPEN) return
    this.socket.send(
      JSON.stringify({
        message: 'SetRecognitionConfig',
        config: {'additional_vocab': transcription_config.custom_dictionary}
      })
    )
  }
